...
export function SearchClient() {
  ...
  const ranked = useMemo(() => rankRestaurants(restaurantsWithDistance, profile, intent, range, history), [restaurantsWithDistance, profile, intent, range, history]);
  function handleRangeChange(nextRange: SearchRange) { setRange(nextRange); storage.saveRange(nextRange); }
  if (!ready) return <AppShell />;
  return <AppShell withBottomPadding><main className="mx-auto flex max-w-5xl flex-col gap-6 px-4 py-5 sm:px-6 lg:py-8"><header><p className="text-sm font-black text-[#7b5545]">Buscar</p><h1 className="mt-1 text-3xl font-black leading-tight">Cuentame tu antojo</h1></header><SearchBox compact initialValue={query} key={query} /><section className="space-y-3"><div className="flex gap-2 overflow-x-auto pb-1">{examples.map((example) => <button className="shrink-0 rounded-full bg-white px-4 py-3 text-sm font-black text-[#5f463b] shadow-sm ring-1 ring-black/5" key={example} onClick={() => router.push(`/search?q=${encodeURIComponent(example)}`)} type="button">{example}</button>)}</div></section><section className="space-y-3 rounded-3xl bg-white p-4 shadow-sm"><RangeSelector onChange={handleRangeChange} value={range} /><div className="flex flex-col gap-3 rounded-2xl bg-[var(--surface-muted)] p-3 text-sm font-bold text-[var(--muted-strong)] sm:flex-row sm:items-center sm:justify-between"><span>{liveLocation.status === "watching" ? `Ubicacion activa +/- ${Math.round(liveLocation.location?.accuracy ?? 0)} m` : "Usa tu ubicacion para filtrar por distancia real"}</span><button className="min-h-11 rounded-2xl bg-[var(--foreground)] px-4 text-sm font-black text-[var(--background)]" onClick={liveLocation.status === "watching" ? liveLocation.stop : liveLocation.request} type="button">{liveLocation.status === "watching" ? "Pausar" : liveLocation.status === "requesting" ? "Solicitando..." : "Usar ubicacion"}</button></div></section>{intent ? <section className="space-y-3 rounded-3xl bg-[#251611] p-4 text-white"><p className="text-xs font-black uppercase tracking-[0.16em] text-white/60">Consulta original</p><h2 className="text-xl font-black">&quot;{intent.query}&quot;</h2><div className="flex flex-wrap gap-2">{compactIntentLabels(intent).map((item) => <span className="rounded-full bg-white/12 px-3 py-1 text-xs font-black" key={item}>{item}</span>)}</div></section> : null}<section className="space-y-3"><h2 className="text-xl font-black">{intent ? "Resultados filtrados" : "Prueba una busqueda"}</h2><div className="grid gap-4 md:grid-cols-2">{ranked.slice(0, intent ? 12 : 6).map((restaurant) => <RestaurantCard key={restaurant.id} restaurant={restaurant} />)}</div></section>{history.length ? <section className="space-y-3"><div className="flex items-center gap-2 text-sm font-black text-[#7b5545]"><Clock size={17} />Historial reciente</div><div className="flex flex-wrap gap-2">{history.slice(0, 6).map((item) => <PreferenceChip key={item.id} label={item.query} onClick={() => router.push(`/search?q=${encodeURIComponent(item.query)}`)} />)}</div></section> : null}</main><BottomNav /></AppShell>;
}
